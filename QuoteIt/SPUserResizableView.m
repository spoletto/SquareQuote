...
@synthesize isBrickable, isShowingBrick;
...
- (void)setIsShowingBrick:(BOOL)bricked {
    isShowingBrick = bricked;
    if (isShowingBrick) {
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    if (self.isShowingBrick) {
        // Custom event handling.
        touchStart = [[touches anyObject] locationInView:self];
        anchorPoint = SPUserResizableViewNoResizeAnchorPoint;
        return;
    }
...
- (void)translateUsingTouchLocation:(CGPoint)touchPoint {
    ...
    if ([self.delegate respondsToSelector:@selector(userResizableViewDidTranslate:)]) {
        [self.delegate userResizableViewDidTranslate:self];
    }
}
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    if (self.isShowingBrick) {
        // Custom event handling.
        CGFloat deltaY = touchStart.y - [[touches anyObject] locationInView:self].y;
        if (deltaY >= 15) {
            if ([self.delegate respondsToSelector:@selector(brickViewWasDraggedUpwards:finalLocation:)]) {
                [self.delegate brickViewWasDraggedUpwards:self finalLocation:[[touches anyObject] locationInView:self.superview]];
            }
            touchStart = [[touches anyObject] locationInView:self];
            anchorPoint = SPUserResizableViewNoResizeAnchorPoint;
        }
