...
- (void)refreshActivityIndicator {
    ...
    if (!active) {
        [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(updateNetworkActivityIndicatorVisibility) object:nil];
        [self performSelector:@selector(updateNetworkActivityIndicatorVisibility) withObject:nil afterDelay:0.25];
    } else {
